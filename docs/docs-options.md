...
Override the default `cwd` for files included by using `[%= include('foo.md') %]`. By default, the `include` mixin will look for files in `./node_modules/{%= name %}/tasks/templates` directory, where some starter templates are stored. ([Also see examples →](./DOCS.md#examples))
