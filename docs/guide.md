...
> {%= name %} makes it easy to build project documentation using simple markdown templates, with no configuration required.


## How does it work?

Throw some markdown files in the `docs` directory of your project and Phaser will use them to [generate your documentation](#core-concepts).
...
_Although zero configuration is required, Phaser has a robust [API](#TODO) and is highly configurable via options._
...
{%= name %} has built-in support for the following template concepts:
...
Before continuing on, ensure that the filepath to your newly created partial, `head.hbs`, is specified in your project's configuration so {%= name %} can take note of it, ensuring that the partial can be used in your templates.

Now, to actually use the partial, add the `{{> head }}` template to the `head` section of your layout where the code was removed. {%= name %} makes this simple by allowing you to use the name of the file you just created as the name of the partial:
...
> Content is written in an easy-to-read plain text format such as markdown. {%= name %} can be extended to convert any format.

Additionally, {%= name %} can convert your content to HTML according to your preferences:
