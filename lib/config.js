...
module.exports = function(verb, base, env) {
   ...
   *   `update` this mapping is setup to be called by the cli
   *           `--config` command, and is used to extend, add,
   *           delete or otherwise update any given property
  ...
  verb.config
    ...
    .map('update', function(val) {
      ...
      try {
        ...
        verb.cache.data = utils.merge({}, verb.cache.data, file.json.verb.data);
      } catch (err) {
        throw new Error('config', 'cannot update pkg.verb', err.stack);
      }
    });
