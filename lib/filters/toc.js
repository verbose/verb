...
module.exports = function(config, options, params) {
  ...
  exports.toc = function(src, options) {
    ...
    var tocOpts = _.defaults({template: tmpl}, options, opts.toc);

    if(src) {
      return phaser.expand(src, opts.glob).map(function(filepath) {
        // Build a relative link to each file
        var link = relative(opts.dest || opts.destBase, filepath);
      ...
      return toc(params.page.content);
    }
  };
  return exports;
};