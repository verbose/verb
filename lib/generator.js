...
module.exports = function(verb, base) {
  ...
  verb.task('init', { silent: true }, function(cb) {
    ...
    verb.ask('init', {save: false}, function(err, answers) {
      ...
      switch (answers.init) {
        case 'defaults':
          var defaults = verb.globals.get('defaults');
  ...
  verb.register('store', function(app) {
    app.enable('silent');

    app.task('defaults', function(cb) {
      var defaults = app.globals.has('defaults.defined');
      ...
      app.ask('defaults', function(err, answers) {
