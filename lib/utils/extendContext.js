  ...
  if (_.isString(patterns) || _.isArray(patterns)) {
    ...
    patterns.forEach(function(pattern) {
      ...
      if (!globlike) {
        ...
        pattern = verb.cwd(pattern);
        try {
          _.extend(obj, require(pattern)(verb));
          verb.verbose.success('Registered local file:', pattern);
        } catch (e) {
            verb.verbose.warn('[verb]: `'+ pattern +'` should export a function with `verb` as the only argument.', e);
        }
      }
